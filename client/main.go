...
func getRequest() {
	...
	// Docker内部での通信では、localhostではなく、サービス名やコンテナ名を使用します。
...
func getRequestWithQueryParam() {
	...
	// http.Get関数を用いて"http://localhost:18888"にリクエストを送信しています。
...
func postFormData() {
	...
	// http.PostForm関数を利用して作成した values を含む POST リクエストを "http://localhost:18888" へ送信します。
...
func postFile() {
	// os.Open 関数を使用して、 server.go というファイルを開いています。
	...
	// http.Post 関数を使い、先ほど開いた server.go ファイルの内容を http://localhost:18888 へPOSTリクエスト送信します。
...
func postText() {
	...
	// http.Post関数を使って、先ほど作成したstrings.Reader（reader）から取得したデータをhttp://localhost:18888へPOSTリクエスト送信します。
...
func postMultipartData() {
	...
	// 全ての書き込みが完了した後、writerを閉じます。これにより、パートの終了を示すバウンダリを出力します。
	writer.Close()

	// 作成したマルチパート形式のHTTPリクエストボディを含むHTTP POSTリクエストをhttp://localhost:18888に対して送信します。
...
func getRequestWithCookies() {
	...
	// このHTTPクライアントを使ってhttp://localhost:18888/cookieへ2回GETリクエストを送信します。
...
func getThroughProxy() {
	...
	// 全てのHTTPリクエストが指定したプロキシURL(http://localhost:18888)を経由するように設定しています。
	client := http.Client{
	...
	// 作成したHTTPクライアントを用いてhttp://github.comへHTTP GETリクエストを送信します。
...
func getFileThroughProtocol() {
	...
	// 作成したHTTPクライアントを用いて file://./main.go へHTTP GETリクエストを送信します。
	// ここでの file://./main.go は、現在のディレクトリ下にある main.go ファイルを指しています。
	resp, err := client.Get("file://./server.go")
	if err != nil {
		panic(err)
	}
...
func deleteRequest() {
	...
	// ここでは、DELETEメソッドを使用してhttp://localhost:18888を対象とするリクエストを作成しています。
